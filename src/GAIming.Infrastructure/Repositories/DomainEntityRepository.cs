...
public class DomainEntityRepository : Repository<DomainEntity>, IDomainEntityRepository
{
    ...
    /// <inheritdoc />
    public async Task<PaginatedResponse<DomainEntity>> SearchEntitiesAsync(string? searchTerm, int page = 1, int pageSize = 20)
    {
        var query = _dbSet.AsQueryable();
        ...
        return new PaginatedResponse<DomainEntity>
        {
            Items = items,
            TotalCount = totalCount,
            Page = page,
    ...
    /// <inheritdoc />
    public async Task<DomainEntityStatistics> GetStatisticsAsync()
    {
        return await GetStatisticsAsync(CancellationToken.None);
    }

    /// <inheritdoc />
    public async Task<IEnumerable<DomainEntity>> GetEntitiesByTypeAsync(string entityType)
    {
        return await _dbSet
            .Where(e => e.Category == entityType)
            .OrderBy(e => e.Name)
            .ToListAsync();
    }
}
