...
public class EmailService : IEmailService
{
    ...
    /// <inheritdoc />
    public async Task<bool> SendDataExportEmailAsync(string to, string fileName, string filePath, int recordCount, CancellationToken cancellationToken = default)
    {
        try
        {
            var subject = $"Data Export Complete - {fileName}";
            var body = $@"
                <h2>Data Export Complete</h2>
                <p>Your data export has been completed successfully.</p>
                <p><strong>File:</strong> {fileName}</p>
                <p><strong>Records:</strong> {recordCount:N0}</p>
                <p><strong>Export Date:</strong> {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss} UTC</p>
                <p>The file is available for download at the provided location.</p>
            ";
        ...
        {
            _logger.LogError(ex, "Failed to send data export email to: {Recipient}", to);
            return false;
        }
    }

    /// <inheritdoc />
    public async Task<bool> SendDataExportErrorEmailAsync(string to, string errorMessage, CancellationToken cancellationToken = default)
    {
        try
        {
            var subject = "Data Export Failed";
            var body = $@"
                <h2>Data Export Failed</h2>
                <p>Unfortunately, your data export request could not be completed.</p>
        ...
        {
            _logger.LogError(ex, "Failed to send data export error email to: {Recipient}", to);
            return false;
        }
    }
